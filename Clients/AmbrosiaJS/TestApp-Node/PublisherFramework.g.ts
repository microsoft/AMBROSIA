// Generated publisher-side framework for the 'TestApp' Ambrosia Node app/service.
// Note: This file was generated on 2021/07/18 at 23:21:17.130.
...
// TODO: It's recommended that you move this namespace to your input file (./PI.ts) then re-run code-gen
                    ...
                    case IC.POST_METHOD_ID:
                        try
                        {
                            ...
                            switch (methodName)
                            {
                                case "ComputePI":
                                    {
                                        const digits: PTM.Published.PI.Digits = IC.getPostMethodArg(rpc, "digits?");
                                        IC.postResult<number>(rpc, PTM.Published.PI.ComputePI(digits));
                                    }
                                    break;
                                
                                case "Sum":
                                    {
                                        const numbers: number[] = IC.getPostMethodArg(rpc, "numbers");
                                        IC.postResult<number>(rpc, PTM.Published.PI.Sum(...numbers));
                                    }
                                    break;
                                
                                case "SetApartmentNumber":
                                    {
                                        const aptNumber: number | string = IC.getPostMethodArg(rpc, "aptNumber");
                                        IC.postResult<number | string>(rpc, PTM.Published.PI.SetApartmentNumber(aptNumber));
                                    }
                                    break;
                                
                                case "TemplateStringTest":
                                    {
                                        const template: `Hello ${"World" | "Universe"}` = IC.getPostMethodArg(rpc, "template");
                                        IC.postResult<`Hello ${"World" | "Universe"}`>(rpc, PTM.Published.PI.TemplateStringTest(template));
                                    }
                                    break;
            ...
            case Messages.DispatchedMessageType.AppEvent:
                ...
                switch (appEvent.eventType)
                {
                    case Messages.AppEventType.ICStarting:
                        ...
                        Meta.publishPostMethod("Sum", 1, ["...numbers: number[]"], "number");
                        Meta.publishPostMethod("SetApartmentNumber", 1, ["aptNumber: number | string"], "number | string");
                        Meta.publishPostMethod("TemplateStringTest", 1, ["template: `Hello ${\"World\" | \"Universe\"}`"], "`Hello ${\"World\" | \"Universe\"}`");
                    ...
                    case Messages.AppEventType.ICStopped:
                        // TODO: Add an exported [non-async] function 'onICStopped(exitCode: number): void' to ./PI.ts, then (after the next code-gen) a call to it will be generated here
                        break;
                    ...
                    case Messages.AppEventType.RecoveryComplete:
                        // TODO: Add an exported [non-async] function 'onRecoveryComplete(): void' to ./PI.ts, then (after the next code-gen) a call to it will be generated here
                        break;
                    ...
                    case Messages.AppEventType.UpgradeState:
                        ...
                        // Note: You will need to import Ambrosia to PI.ts in order to reference the 'Messages' namespace.
                    ...
                    case Messages.AppEventType.FirstStart:
                        // TODO: Add an exported [non-async] function 'onFirstStart(): void' to ./PI.ts, then (after the next code-gen) a call to it will be generated here
                        break;

                    case Messages.AppEventType.BecomingPrimary:
                        PTM.AppEventHandlers.onBecomingPrimary();
                        break;
