// Generated consumer-side API for the 'ASTTest' Ambrosia Node app/service.
...
const _knownDestinations: string[] = []; // All previously used destination instances (the 'ASTTest' Ambrosia app/service can be running on multiple instances, potentially simultaneously); used by the postResultDispatcher (if any)
 ...
 * Must be called at least once (with the name of a registered Ambrosia instance that implements the 'ASTTest' API) before any other method in the API is used.
 */
...
function checkDestinationSet(): void
{
    ...
    {
        throw new Error("setDestinationInstance() must be called to specify the target destination before the 'ASTTest' API can be used.");
    }
}
...
export function postResultDispatcher(senderInstanceName: string, methodName: string, methodVersion: number, callID: number, callContextData: any, result: any, errorMsg: string): boolean
{
    ...
    {
        return (false); // Not handled: this post result is from a different instance than the destination instance currently (or previously) targeted by the 'ASTTest' API
    }
