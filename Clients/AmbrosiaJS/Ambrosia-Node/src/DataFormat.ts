...
export function readInt64Fixed(bytes: Uint8Array | Buffer, startIndex: number = 0): bigint
{
    ...
    {
        let buffer: Buffer = Buffer.from(bytes.slice(startIndex, startIndex + 8)); // TODO: Would using bytes.subarray() be faster?
...
export function writeInt64FixedNumber_NoBuffer(value: number): Uint8Array
{
    ...
    // Encode as little-endian without using roll left/right or other bitwise operators (because these operators automatically cast operands to signed 32-bit values)
    if (value < 0)
    {
...
export function readInt64FixedNumber_NoBuffer(bytes: Uint8Array): number
{
    ...
    // Decode as little-endian without using roll left/right or other bitwise operators (because these operators automatically cast operands to signed 32-bit values)
    if (isNegativeValue)
    {
...
// A VarInt is a byte-encoding scheme [for integer values] where the most-significant bit is used to indicate that the next byte is also part of the encoded value.
// The low 7-bits of a byte are value bits. Byte 0 stores the least significant 7-bits of the value, and the last byte (whose most-significant bit will be 0) stores
...
// For example: ZZ 0 = 0, ZZ 1 = -1, ZZ 2 = 1, ZZ 3 = -2, ZZ 4 = 2, etc. So the VarInt encoding for ZZ 300 above actually encodes integer value 150.
// For more on VarInt and ZigZag encoding see https://developers.google.com/protocol-buffers/docs/encoding. 
// The varIntResult has 2 properties: number of bytes read ('length'), and the integer value ('value'). The 'value' is (by design) a JS number to 
...
function readVarInt(bytes: Uint8Array | Buffer, startIndex: number, numBits: number): varIntResult
{
    ...
    do
    {
        ...
        else
        {
            ...
            //   9007199254740991  : Number.MAX_SAFE_INTEGER (Math.pow(2, 53) - 1 [or 8 petabytes])
    ...
    {
        // Note: We can't use any bitwise operators here because they automatically cast operands to signed 32-bit values
        if (zigZagValue % 2 === 0) 
        {
...
export function writeVarInt64(value: number): Uint8Array
{
    ...
    // Note: When we convert the value to ZigZag format we MUST do so without using roll left/right or other bitwise operators (because these operators automatically cast operands to signed 32-bit values)
...
export function runVarIntTests(): void
{
    ...
    Utils.log("Running VarInt 64-bit size checks...");
    // 1 byte has 7 value bits, so it can hold values up to 127, which for ZigZag encoded integers means up to +63 (ZZ 63 = 126, ZZ -64 = 127). So the spillover value to 2 bytes is +64).
    // So the maximum [ZigZag encode] value that can be stored in n bytes is: (Math.pow(2, n * 7) / 2) - 1
    // For example, for n = 3 the maximum is (Math.pow(2, 21) / 2) - 1 = 1,048,575
    for (let n = 1; n <= 7; n++) // 7 * 7 = 49, which keeps us under Number.MAX_SAFE_INTEGER (2^53 - 1)
    {
        const maxValue: number = (Math.pow(2, n * 7) / 2) - 1;
        let decoded32: varIntResult = readVarInt64(writeVarInt64(maxValue));
        if (decoded32.length !== n)
        {
            throw new Error(`The largest value storable in ${n} bytes (${maxValue}) unexpectedly required ${decoded32.length} bytes`);
        }
        decoded32 = readVarInt64(writeVarInt64(maxValue + 1));
        if (decoded32.length !== n + 1)
        {
            throw new Error(`The first value requiring ${n + 1} bytes (${maxValue + 1}) unexpectedly required ${decoded32.length} bytes`);
        }
    }
 ...
 * The default pool size (set via constructor) is 2MB.
 */
export class BytePool
{
    private _pool: Uint8Array; // Initialized in constructor
    ...
    /** The size (in bytes) of the active block, or -1 if there's no currently active block. */
    get activeBlockSize(): number
    {
        return ((this._blockStartPos === -1) ? -1 : this._pos - this._blockStartPos);
    }
    ...
    /** 
     * Ends a block of contiguous allocation, and returns the block (copied, by default, from the pool). Throws if there is no active block.\
    ...
    endBlock(copy: boolean = true): Uint8Array
    {
        if (this._blockStartPos === -1)
        {
            throw new Error("There is no active block; endBlock() must not be called without first calling startBlock()");
        }
    ...
    private canAddToBlock(caller: string, addLength: number): void
    {
        if (this._blockStartPos === -1)
        {
    ...
    /** Adds (copies) the supplied buffer to the current block. Returns the number of bytes added. */
    ...
    /** Adds (copies) the supplied bytes to the current block. Returns the number of bytes added. */
    ...
    /** Adds (copies) the bytes for a VarInt32 (for the specified value) to the current block. Returns the number of bytes added. */
