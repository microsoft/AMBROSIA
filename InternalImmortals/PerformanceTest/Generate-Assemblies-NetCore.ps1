...
Write-Host "Executing codegen command: dotnet $AMBROSIATOOLS\$AMBVARIANTCORE\AmbrosiaCS.dll CodeGen -a=API\bin\$AMBVARIANTCORE\ServerAPI.dll -a=ClientAPI\bin\$AMBVARIANTCORE\ClientAPI.dll -p=API\ServerAPI.csproj -p=ClientAPI\ClientAPI.csproj -o=PTAmbrosiaGeneratedAPI -f=net46 -f=netcoreapp2.2"
