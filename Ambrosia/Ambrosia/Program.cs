...
//                    if (reconnecting) // BUGBUG We are temporarily disabling this optimization which avoids unnecessary locking as reconnecting is not a sufficient criteria: We found a case where input is arriving during reconnection where counting was getting disabled incorrectly. Further investigation is required.
    ...
    public class AmbrosiaRuntime : VertexBase
    {
        ...
        internal class MachineState
        {
            ...
            internal long LastCommittedCheckpoint { get; set; }
            internal long LastLogFile { get; set; }
            internal Committer Committer { get; set; }
            internal ConcurrentDictionary<string, InputConnectionRecord> Inputs { get; set; }
            internal ConcurrentDictionary<string, OutputConnectionRecord> Outputs { get; set; }
        }
        ...
        private void RecoverShard(long shardID, long checkpointToLoad, bool testUpgrade)
        {
        ...
        private async Task RecoverShardsAsync(long[] recoverFromShards, long checkpointToLoad = -1, bool testUpgrade = false)
        {
            ...
            {
                threads[i] = new Thread(() => RecoverShard(recoverFromShards[i], checkpointToLoad, testUpgrade)) { IsBackground = true };
            }
        ...
        public void Initialize(object param, long[] recoverFromShards, long shardID, Func<long, long> shardMap)
        {
            ...
            {
                RecoverShardsAsync(recoverFromShards).GetAwaiter();
            }
        }
